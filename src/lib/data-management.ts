...
export async function exportData(): Promise<BackupData> {
  ...
  const settingsRows = await db.select<Array<{
    theme: string;
    currency: string;
    notification_enabled: number;
    notification_days_before: string;
  }>>('SELECT theme, currency, notification_enabled, notification_days_before FROM settings WHERE id = ?', ['singleton']);
...
export async function importData(data: BackupData, options: { 
    ...
    for (const category of data.categories) {
      ...
      await db.execute(
        `INSERT OR REPLACE INTO categories (id, name, color, icon, is_default, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [category.id, category.name, category.color, category.icon, 0, category.created_at || new Date().toISOString()]
      );
    }
    ...
    for (const sub of data.subscriptions) {
      await db.execute(
        `INSERT OR REPLACE INTO subscriptions 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          sub.id, sub.name, sub.amount, sub.currency, sub.billing_cycle,
          sub.billing_day, sub.next_payment_date, sub.category_id,
          sub.color, sub.logo_url, sub.notes, sub.is_active ? 1 : 0,
    ...
    for (const payment of data.payments) {
      await db.execute(
        ...
        [
          ...
          payment.paid_at, payment.due_date, payment.status,
          payment.notes, payment.created_at || new Date().toISOString()
        ]
      );
    }
