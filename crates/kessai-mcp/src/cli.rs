...
use kessai_core::models::{BillingCycle, NewCategory, NewSubscription, UpdateSubscription};
use kessai_core::KessaiCore;
...
pub fn run(command: Command, core: &KessaiCore) -> anyhow::Result<()> {
    match command {
...
fn cmd_list(core: &KessaiCore) -> anyhow::Result<()> {
    ...
    for sub in &subs {
        ...
        let next = sub.next_payment_date.as_deref().unwrap_or("-");
    ...
    println!("{table}");
    println!("\n{} subscription(s) total", subs.len().to_string().bold());
    Ok(())
}
...
fn cmd_add(
    ...
    let sub = core.subscriptions().create(NewSubscription {
        ...
        status: kessai_core::models::SubscriptionStatus::Active,
        trial_end_date: None,
        shared_count: 1,
        is_pinned: false,
    })?;
...
fn cmd_remove(core: &KessaiCore, id: String) -> anyhow::Result<()> {
...
fn cmd_toggle(core: &KessaiCore, id: String) -> anyhow::Result<()> {
...
fn cmd_pay(core: &KessaiCore, sub_id: String, due_date: String, amount: f64) -> anyhow::Result<()> {
...
fn cmd_categories(core: &KessaiCore) -> anyhow::Result<()> {
...
fn cmd_upcoming(core: &KessaiCore, days: i64) -> anyhow::Result<()> {
    let subs = core.subscriptions().list()?;
    let upcoming = kessai_core::utils::get_upcoming_subscriptions(&subs, days);

    if upcoming.is_empty() {
        println!(
            "{}",
            format!("No upcoming payments in the next {days} days.").dimmed()
        );
        return Ok(());
    }
...
fn cmd_stats(core: &KessaiCore) -> anyhow::Result<()> {
    let subs = core.subscriptions().list()?;
    let cats = core.categories().list()?;
    ...
    let total_monthly: f64 = active
        .iter()
        .map(|s| kessai_core::utils::calculate_monthly_amount(s.amount, &s.billing_cycle))
        .sum();
    ...
    println!("  Active:         {} / {}", active.len(), subs.len());
    if !active.is_empty() {
        println!(
            "  Avg per sub:    {:.2} {}",
    ...
    for sub in &active {
        let monthly = kessai_core::utils::calculate_monthly_amount(sub.amount, &sub.billing_cycle);
        let key = sub
            .category_id
            .clone()
...
fn cmd_export(core: &KessaiCore, output: Option<PathBuf>) -> anyhow::Result<()> {
...
fn cmd_import(core: &KessaiCore, file: PathBuf, clear: bool) -> anyhow::Result<()> {
    let content = std::fs::read_to_string(&file)?;
    let backup: kessai_core::models::BackupData = serde_json::from_str(&content)?;
