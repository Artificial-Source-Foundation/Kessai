...
struct AddSubscriptionParams {
    ...
    notes: Option<String>,
    #[schemars(
        description = "Status: trial, active, paused, pending_cancellation, grace_period, cancelled"
    )]
...
#[tool_router]
impl KessaiMcp {
    pub fn new(core: KessaiCore) -> Self {
        Self {
            core,
    ...
    async fn get_spending_by_category(&self) -> Result<CallToolResult, McpError> {
        ...
        for sub in &active {
            let monthly =
                kessai_core::utils::calculate_monthly_amount(sub.amount, &sub.billing_cycle);
            let key = sub
                .category_id
                .clone()
    ...
    #[tool(description = "Export all Kessai data as a JSON backup")]
    ...
    async fn import_data(
        ...
        let backup: kessai_core::models::BackupData = serde_json::from_str(&p.data)
...
#[tool_handler]
impl ServerHandler for KessaiMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            server_info: Implementation {
                name: "kessai-mcp".into(),
                title: None,
                version: env!("CARGO_PKG_VERSION").into(),
                description: Some("MCP server for Kessai subscription tracker".into()),
                icons: None,
                website_url: None,
            },
            instructions: Some(
                "Kessai is a personal subscription tracker. Use tools to manage subscriptions, \
    ...
    async fn list_resources(
        ...
        Ok(ListResourcesResult {
            ...
            resources: vec![
                ...
                make_resource("kessai://settings", "Settings", "Current app settings"),
                make_resource("kessai://cards", "Payment Cards", "All payment cards"),
                make_resource(
                    "kessai://stats/dashboard",
                    "Dashboard Stats",
        ...
        let json = match uri.as_str() {
            ...
            "kessai://stats/dashboard" => {
                ...
                let total_monthly: f64 = active
                    .iter()
                    .map(|s| {
                        kessai_core::utils::calculate_monthly_amount(s.amount, &s.billing_cycle)
                    })
                    .sum();
            ...
            _ if uri.starts_with("kessai://subscriptions/") => {
                let id = uri.strip_prefix("kessai://subscriptions/").unwrap();
                let sub = self
                    .core
                    .subscriptions()
                    .get(id)
            ...
            _ if uri.starts_with("kessai://payments/") => {
                let parts: Vec<&str> = uri
                    .strip_prefix("kessai://payments/")
                    .unwrap()
                    .split('/')
                    .collect();
                if parts.len() != 2 {
                    return Err(mcp_err("Expected format: kessai://payments/{year}/{month}"));
                }
        ...
        Ok(ListResourceTemplatesResult {
            ...
            resource_templates: vec![
                make_resource_template(
                    "kessai://subscriptions/{id}",
                    "Subscription Details",
                    "Get a single subscription by ID",
                ),
                make_resource_template(
                    "kessai://payments/{year}/{month}",
                    "Monthly Payments",
        ...
        match request.name.as_str() {
            ...
            "upcoming_payments" => {
                ...
                let settings = self
                    ...
                    .get()
                    .map_err(|e| mcp_err(e.to_string()))?;
                let upcoming = kessai_core::utils::get_upcoming_subscriptions(&subs, 30);
